...
|---|---|---|---|---|---|---|---|
| 2026-06-15 | Sessão experimental — design de skills client-side e acesso fluido ao ecossistema | **Skills client-side / Skills portáteis** — cada workflow do ecossistema pode ter uma versão instalável projetada para ser copiada na ferramenta do usuário (ex: Claude). Essa versão: (a) sabe onde buscar o padrão canônico no ecossistema via API; (b) tem o padrão embutido como fallback para operação sem token; (c) verifica automaticamente se houve atualização de versão e alerta o usuário; (d) opera em três modos — sem token (fallback), token leitura (verifica + atualiza), token escrita (deposita no ecossistema). Terminologia aprovada: **client-side** (skill na ferramenta do usuário) vs **server-side** (skill canônica no ecossistema). O repositório que agrega todas as versões instaláveis seria do **tipo B — repositório dedicado** (centralizado, fácil de descobrir, com mecanismo de alerta de desatualização). Protótipo construído nesta sessão: skill de registro de reunião (S06) com os três modos de operação. Impacto: afeta taxonomia de tipos do ecossistema (novo tipo ou subpasta?), nomenclatura, e o próprio S01 (skl-criador-de-skills). Registro desta ideia é pré-requisito para processar em W04 antes de qualquer execução. | #2, #6, #7 | ✅ confirmado | `2026-06-15-skills-client-side-e-rascunhos-staging.md` | ⏳ pendente | 2026-06-15 |
| 2026-06-15 | Sessão experimental — design de skills client-side e acesso fluido ao ecossistema | **Pasta rascunhos-staging no hub-entrada** — criação de `hub-entrada/rascunhos-staging/` como repositório temporário para documentos MD produzidos em sessões de amadurecimento de ideias em staging. Problema resolvido: ideias ricas em conteúdo eram comprimidas em uma linha de tabela ao entrar na Seção C, perdendo o material elaborado nas sessões longas. Design: (a) entrada sem atrito — rascunho é opcional, não obrigatório; (b) tabela Seção C ganha coluna `Rascunho` com valor `—` ou nome do arquivo; (c) nomenclatura `AAAA-MM-DD-[slug].md`; (d) ciclo de vida: migra para destino definitivo quando aprovada em W04, permanece como histórico se arquivada. Impactos: alteração estrutural da tabela Seção C (já executada nesta sessão), atualização do rito do agente (S04/W04), política de limpeza da pasta. Inaugurada nesta mesma sessão com o rascunho `2026-06-15-skills-client-side-e-rascunhos-staging.md`. | #1, #6 | ✅ confirmado | `2026-06-15-skills-client-side-e-rascunhos-staging.md` | ⏳ pendente | 2026-06-15 |
